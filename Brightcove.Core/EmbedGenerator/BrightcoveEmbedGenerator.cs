...
namespace Brightcove.Core
{
    public class BrightcoveEmbedGenerator
    {
        ...
        readonly static string jsTemplate = "<div style='width: {4}px;'><video-js data-account='{0}' data-player='{1}' data-embed='default' controls='' data-video-id='{2}' data-playlist-id='{3}' data-application-id='' width='{4}' height='{5}' class='vjs-fluid'></video-js></div>";
        readonly static string jsResponsiveTemplate = "<div style='max-width: {4}px;'><style>video-js.video-js.vjs-fluid:not(.vjs-audio-only-mode) {{padding-top: {5}%;}}</style><video-js data-account='{0}' data-player='{1}' data-embed='default' controls='' data-video-id='{2}' data-playlist-id='{3}' data-application-id='' class='vjs-fluid'></video-js></div>";
        ...
        protected EmbedMarkup GenerateIframe(EmbedModel model)
        {
            ...
            switch(model.MediaSizing)
            {
                case MediaSizing.Responsive:
                    string aspectRatio = ((double)model.Width / model.Height * 100.0).ToString("F2");
        ...
        protected EmbedMarkup GenerateJavaScript(EmbedModel model)
        {
            ...
            switch (model.MediaSizing)
            {
                case MediaSizing.Responsive:
                    string aspectRatio = ((double)model.Width / model.Height * 100.0).ToString("F2");
                    result.Markup = string.Format(jsResponsiveTemplate, model.AccountId, model.PlayerId, videoId, playlistId, model.Width, aspectRatio);
                    break;
                case MediaSizing.Fixed:
                    result.Markup = string.Format(jsTemplate, model.AccountId, model.PlayerId, videoId, playlistId, model.Width, model.Height);
                    break;
                default:
